...
export class BookingService {
  ...
  const { propertyId, userId, startDate, endDate, totalAmount, specialRequests } = createBookingDto;
  ...
  try {
    ...
    if (conflictingBooking) {
      this.logger.warn(`Property ${property.id} is not available for selected dates`);
    ...
    this.logger.log(`Monthly invoices generated for booking id ${savedBooking.id}`);
  ...
  async updateStatus(id: string, status: BookingStatus): Promise<Booking> {
    this.logger.log(`Updating booking status to ${status} for booking id: ${id}`);
    try {
      ...
      this.logger.log(`Booking status updated to ${status} for booking id: ${id}`);
      return booking;
    } catch (error) {
  ...
  async hasUserBookedProperty(userId: string, propertyId: string): Promise<boolean> {
